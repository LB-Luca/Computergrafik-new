...
void main()
{
	...
	vec3 light_dir;
	/* light source unendlich weit weg */
	if (light_pos.w == 0) {
		light_dir = normalize(viewspace_light_pos);
	}
	else {
		light_dir = normalize(viewspace_light_pos - viewspace_pos);
	}
	...
	/* Diffuse Part */
	float NdotL = max(dot(viewspace_normal, light_dir), 0.0f);
