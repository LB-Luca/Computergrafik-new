...
PipelineInfo create_ray_tracing_pipeline(VkalInfo * vkal_info, uint32_t model_count, uint32_t material_count, uint32_t texture_count)
{
    ...
    */
    ...
    read_shader_file("/raygen.spv", &shader_code, &shader_size);
    ...
    /* Ray Miss Group */
    read_shader_file("/raymiss.spv", &shader_code, &shader_size);
    ...
    read_shader_file("/lightmiss.spv", &shader_code, &shader_size);
    shader_stages.push_back(vkal_create_shader(shader_code, shader_size, VK_SHADER_STAGE_MISS_BIT_KHR));
    ...
    read_shader_file("closesthit.spv", &shader_code, &shader_size);
...
{   
    ...
    memcpy(index_buffer.mapped, model.indices_32, model.index_count * sizeof(uint32_t));
...
int main(int argc, char** argv)
{
    ...
    std::string shaders_dir = "../../A5_Raytracing/shaders";
    if (argc > 1) {
        assets_dir = argv[1];
    }
    if (argc > 2) {
    ...
    wanted_features.features2.features.shaderInt16 = VK_TRUE;
    ...
    /* Textures */
    Image sandImage = load_image("textures/sand_diffuse.jpg");
    Image knightImage = load_image("textures/knight.png");
    ...
    /* Geometry */
    Model palmtree = create_model_from_file_indexed_rt("obj/palmtree.obj");
    ...
    Model plane = create_model_from_file_indexed_rt("obj/plane.obj");
    ...
    Model knight = create_model_from_file_indexed_rt("obj/pknight_small.obj");
